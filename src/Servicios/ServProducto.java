...
public class ServProducto implements ServiciosProductos {
    ...
    @Override
    public List<Producto> consula(String nombre) {
        List<Producto> listaRep = repositorioProductos.listar();
        List<Producto> listaAux = List.of();

        for (Producto producto : listaRep){
            String nombreProductoLower = producto.getNombre().toLowerCase();

            if(nombreProductoLower.equalsIgnoreCase(nombre.toLowerCase())){
    ...
    public void eliminacionRestringida(String codigo) {
        ...
        for (Producto producto : listaRep) {
            if (producto.getCodigo().equals(codigo)) {
                ...
                if (asociadoAPedido) {
                    throw new IllegalArgumentException("No se puede eliminar el producto porque está asociado a un pedido");
                } else {
